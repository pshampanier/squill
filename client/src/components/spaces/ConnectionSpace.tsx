...
export default function ConnectionSpace(props: SpaceProps) {
  ...
  const { status, failureCount, refetch } = useQuery(
    "agent-connect",
    async () => {
      // Get the agent connection parameters which are stored in the environment
      const agentConnectionParameters = await env.getAgentConnectionParameters();

      // Connect to the agent
      console.log(`Connecting agent: ${agentConnectionParameters.url}`);
      await Agent.connect(agentConnectionParameters.url, agentConnectionParameters.apiKey);
  ...
  return (
    <Space {...props} className="items-center justify-center">
      <div className="relative bg-transparent w-full items-center">
        {status === "loading" && (
