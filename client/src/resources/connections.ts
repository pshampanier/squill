...
const Connections = {
   ...
   *
   * @param connectionId The identifier of the connection to execute the buffer on.
   ...
   */
  async execute(id: string, origin: string, buffer: string): Promise<QueryExecution[]> {
    return (
      await agent().post<string, QueryExecution>(`/connections/${id}/execute`, buffer, {
        headers: {
